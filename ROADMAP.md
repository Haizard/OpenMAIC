...
platform is built through **Phase A (ingest), B (generation), C (review) and E (quality loop)** —
298 academic tests green. **None of it is pushed: the push needs Haitham's credentials.**

**Next up is Phase D, and it is blocked on content, not code.** It re-points Slices 3, 4 and 6 at
the shared bank. Doing it against an empty bank would break homework that works today, so Haitham
must upload a book, generate, and publish before it starts.
...
- [ ] `academic_assignments.content_item_id` — assignment points at a bank item instead of
      carrying its own question text
- [ ] Deterministic per-student selection: `hash(studentId, setKey)`, stable on reload
- [ ] Shuffled choice order per student, mapped back to the true answer at grading
- [ ] Anti-repetition: skip recently answered items
- [ ] Re-point Slices 3, 4 and 6 delivery at the bank
- [ ] Tests: two students get different sets; one student gets the same set on reload
