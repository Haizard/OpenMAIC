...
**Product status:** Slices 0–6 complete and tested (227 tests, commit 09eb44d, **not yet pushed**
— the push needs Haitham's credentials). Phase A of the AI content platform (source ingest) is
built and tested on top of that, **also not yet committed**.
...
Phases A–C are additive and safe. **Phase D is the real refactor** — it changes how existing,
tested slices get their content — and must not start until the bank holds real generated content.
...
- [ ] Student flags a question → unpublished immediately, back to review
- [ ] Flag rate per item surfaces systematically bad generations
